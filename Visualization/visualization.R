...
broadband %>% 
  ...
  geom_boxplot() +
  labs(title = "Average Download Speed", x = "District",
       y = "Speeds (Mbit/s)")+
  coord_flip()
...
HousePricesclean %>%
  ...
  group_by(District) %>%
  summarise(AveragePrice = mean(Price)) %>%
  ggplot(aes(x = factor(District), y = AveragePrice, group = 1)) +  # Added 'group = 1' for the line
  geom_line(linetype = "dashed", color = "lightgrey", size = 1.5) +  # Used dashed line for better visibility
  ...
  labs(title = "2020-2022 Average House Prices", x = "District", y = "Average Price") +
  theme_minimal()  # Added a theme for a cleaner look
...
schoolData %>%
  ...
  geom_boxplot(color = "orange", fill = "steelblue") +  
